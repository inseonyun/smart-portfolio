...
class TimelinePortfolioDatabaseHelper(context: Context): SQLiteOpenHelper(context, "TimelinePortfolio.db", null, 1) {
    ...
    fun insertData(data: Timeline) {
        ...
        val query = "INSERT INTO TimelinePortfolio('name', 'title', 'contents', 'date', 'image', 'defaultImage', 'url', 'circleColor') " +
                "values('${data.name}', '${data.title}', '${data.contents}', '${data.date}', ?, '${data.defaultImage}', '${data.url}', '${data.circleColor}');"
    ...
    fun updateData(data: Timeline) {
        val db = this.writableDatabase
        val query = "UPDATE TimelinePortfolio set name = '${data.name}', title = '${data.title}', contents = '${data.contents}', date = '${data.date}', image = ?, defaultImage = '${data.defaultImage}', url = '${data.url}', circleColor = '${data.circleColor}' WHERE id = '${data.id}'"
        val p = db.compileStatement(query)
        p.bindBlob(1, data.image)
        p.execute()
        p.close()
    ...
    fun selectData(name: String): ArrayList<Timeline> {
        ...
        while(cursor.moveToNext()) {
            val rowData = Timeline(cursor.getInt(0), cursor.getString(1), cursor.getString(2), cursor.getString(3), cursor.getString(4), cursor.getBlob(5), cursor.getInt(6), cursor.getString(7), cursor.getInt(8))
            result.add(rowData)
        }
        cursor.close()
